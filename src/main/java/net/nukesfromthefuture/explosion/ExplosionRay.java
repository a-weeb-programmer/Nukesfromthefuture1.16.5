...
public class ExplosionRay {
    ...
    public ExplosionRay(World world, int x, int y, int z, int strength, int count, int speed, int length) {
        this.world = world;
        this.posX = x;
        this.posY = y;
        this.posZ = z;
    ...
    public void processTip(int count) {
        ...
        for(int l = 0; l < Integer.MAX_VALUE; l++) {
            ...
            Vector3f vec = new Vector3f(x - this.posX, y - this.posY, z - this.posZ);
            double pX = vec.getX() / vec.hashCode();
            double pY = vec.getY() / vec.hashCode();
            double pZ = vec.getZ() / vec.hashCode();

            for(int i = 0; i < vec.hashCode(); i ++) {
    ...
    public void collectTip(int count) {
        ...
        for(int k = 0; k < count; k++) {
            ...
            for(int i = 0; i < length; i ++) {
                ...
                float x0 = (float) (posX + (vec.getX() * i));
                float y0 = (float) (posY + (vec.getY() * i));
                float z0 = (float) (posZ + (vec.getZ() * i));

                if(!world.getBlockState(new BlockPos((int)x0, (int)y0, (int)z0)).getMaterial().isLiquid())
                    res -= Math.pow(world.getBlockState(new BlockPos((int)x0, (int)y0, (int)z0)).getExplosionResistance(world, new BlockPos((int) x0, (int) y0, (int) z0), null), 1.25);
                else
                    res -= Math.pow(Blocks.AIR.getDefaultState().getExplosionResistance(world, new BlockPos(x0, y0, z0), null), 1.25);

                if(res > 0 && world.getBlockState(new BlockPos((int)x0, (int)y0, (int)z0)) != Blocks.AIR.getDefaultState()) {
    ...
    public void collectTipExperimental(int count) {
        ...
        for(int k = 0; k < count; k++) {
            ...
            for(int i = 0; i < length; i ++) {
                ...
                if(!world.getBlockState(new BlockPos((int)x0, (int)y0, (int)z0)).getMaterial().isLiquid())
                    res -= Math.pow(world.getBlockState(new BlockPos((int)x0, (int)y0, (int)z0)).getExplosionResistance(world, new BlockPos(x0, y0, z0), null), 7.5D - fac);
                else
                    res -= Math.pow(Blocks.AIR.getDefaultState().getExplosionResistance(world, new BlockPos(x0, y0, z0), null), 7.5D - fac);
    ...
    public void collectTipMk4_5(int count) {
        ...
        for(int v = startY; v <= bowCount; v++) {
            ...
            Vector3d heightVec = new Vector3d(0, -strength, 0);
            heightVec.rotateRoll(rot);

            double y = heightVec.getY();
            ...
            for(int r = startCir; r < circumference; r ++) {
                ...
                Vector3d vec = new Vector3d(sectionRad, y, 0);
                vec = vec.normalize();
				/*if(y > 0)
					vec.rotateAroundZ((float) (y / sectionRad) * 0.15F);*/
				/*if(y < 0)
					vec.rotateAroundZ((float) (y / sectionRad) * 0.15F);*/
                vec.rotateYaw((float) (360 / circumference * r));
